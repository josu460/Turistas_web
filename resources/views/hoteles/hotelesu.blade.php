  ...
  @foreach($hoteles as $hotel)
    <li class="py-4 flex flex-col lg:flex-row items-start lg:items-center space-y-4 lg:space-y-0 lg:space-x-6">
        ...
        <div class="flex-1 space-y-2">
            ...
            <button class="text-blue-600 hover:underline dark:text-blue-400" type="button" data-drawer-target="drawer-{{ $hotel->id }}" data-drawer-show="drawer-{{ $hotel->id }}" aria-controls="drawer-{{ $hotel->id }}">
                Ver detalles
            </button>

            <!-- Drawer component -->
            <div id="drawer-{{ $hotel->id }}" class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-96 dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label-{{ $hotel->id }}">
                <h5 id="drawer-label-{{ $hotel->id }}" class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400">
                ...
                <button type="button" data-drawer-hide="drawer-{{ $hotel->id }}" aria-controls="drawer-{{ $hotel->id }}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 right-2.5 flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white">
